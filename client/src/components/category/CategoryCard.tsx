  ...
  return (
    <Link className="group flex h-full min-h-[460px] flex-col overflow-hidden rounded-[1.5rem] border border-ink/5 bg-white shadow-soft transition duration-300 hover:-translate-y-1 hover:shadow-xl" to={`/categories/${category.slug}`}>
      <div className="aspect-[4/3] overflow-hidden">
        <img className="h-full w-full object-cover transition duration-500 group-hover:scale-105" src={category.image} alt={category.name} loading="lazy" decoding="async" sizes="(min-width: 1024px) 25vw, (min-width: 640px) 50vw, 100vw" />
      </div>
